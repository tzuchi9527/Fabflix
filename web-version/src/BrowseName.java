...
@WebServlet(name = "BrowseName", urlPatterns = "/browseName")
public class BrowseName extends HttpServlet {
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            String query = String.format("SELECT m.*, group_concat(distinct s.name), group_concat(distinct s.id), group_concat(distinct g.name), r.rating"
            		...
            		+ " and m.id IN"
            		+ " (SELECT m.id FROM movies m, stars s, stars_in_movies sm"
            		+ " WHERE m.title like '%s' and s.id = sm.starId and m.id = sm.movieId)"
            ...
            */
            ...
            String sql = "SELECT m.*, group_concat(distinct s.name), group_concat(distinct s.id), group_concat(distinct g.name), r.rating"
            		...
            		+ " and m.id IN"
            		+ " (SELECT m.id FROM movies m, stars s, stars_in_movies sm"
            		+ " WHERE m.title like ? and s.id = sm.starId and m.id = sm.movieId)"
            		+ " GROUP BY r.rating, m.id ";
            ...
            out.println("<p><a href=\"./browse_name?numOfpage=10&sort="+sort+"&order="+order+"&name=" + name + "\">10</a>");  
            out.println("<a href=\"./browse_name?numOfpage=25&sort="+sort+"&order="+order+"&name=" + name + "\">25</a>");  
            out.println("<a href=\"./browse_name?numOfpage=50&sort="+sort+"&order="+order+"&name=" + name + "\">50</a>");  
            out.println("<a href=\"./browse_name?numOfpage=100&sort="+sort+"&order="+order+"&name=" + name + "\">100</a></p>");  
  			*/
            ...
            /*
            if (pagenumber-2 >= 0) {
            	out.println("<p>prev: <a href = \"./browse_name?page="+String.valueOf(pagenumber-2)+"&numOfpage="+numOfpage+"&sort="+sort+"&order="+order+"&name="+name+"\">"+String.valueOf(pagenumber-1)+"</a></p>");          	
            }        
            out.println("<p>now: "+ String.valueOf(pagenumber) +"</p>");
            if (rs.next()) {
            	out.println("<p>next: <a href = \"./browse_name?page="+String.valueOf(pagenumber)+"&numOfpage="+numOfpage+"&sort="+sort+"&order="+order+"&name="+name+"\">"+String.valueOf(pagenumber+1)+"</a></p>");
            }
			*/
